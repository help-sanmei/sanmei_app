...
#   #         logger.debug("--------@setsuiri_str = #{calc_setsu(1988)}")
#   #         redirect_to("/")
#   # end  test
	...
	# 計算ボタンイベント
		...
		dt1 = Date.new(b_y0, b_m0-1, b_d0)
		dt2 = Date.new(1912, 2-1, 1)
		diff = dt1 - dt2
	...
	if sangou_flag==0 && special_flag==0
		...
		if isho_flag1+isho_flag2+isho_flag3==3
			...
			if isho_flag1==2
				...
				if (calc_gogyo_hankai(im_sd,im_sy)==1) || (calc_gogyo_hankai(im_sd,im_sy)==2) || (calc_gogyo_hankai(im_sd,im_sy)==3) || (calc_gogyo_hankai(im_sd,im_sy)==4) || (calc_gogyo_hankai(im_sd,im_sy)==5)
					if d_12shiggno[im_sd]!=calc_gogyo_hankai(im_sd,im_sy)
						gg_adj_isho[d_12shiggno[im_sd]] -= 1,gg_adj_isho[calc_gogyo_hankai(im_sd,im_sy)] += 1
					if d_12shiggno[im_sy]!=calc_gogyo_hankai(im_sd,im_sy)
						gg_adj_isho[d_12shiggno[im_sy]] -= 1,gg_adj_isho[calc_gogyo_hankai(im_sd,im_sy)] += 1
				end
			elsif isho_flag3==2
				#月支・年支　
				if (calc_gogyo_hankai(im_sm,im_sy)==1) || (calc_gogyo_hankai(im_sm,im_sy)==2) || (calc_gogyo_hankai(im_sm,im_sy)==3) || (calc_gogyo_hankai(im_sm,im_sy)==4) || (calc_gogyo_hankai(im_sm,im_sy)==5)
					...
					if d_12shiggno[im_sy]!=calc_gogyo_hankai(im_sm,im_sy)
						gg_adj_isho[d_12shiggno[im_sy]] -= 1,gg_adj_isho[calc_gogyo_hankai(im_sm,im_sy)] += 1
				end
			end
	...
	if  (sangou_flag==0) && (toritatu_flag==1) && (special_flag==0)       #１組の場合
	  if gogyo_last[3]>=3
	    ...
	    if im_sm==5
	      gogyo_last[3] += 1,gogyo_last[4] -= 1
	    end  
	  elsif
	    if im_sm==5
	      gogyo_last[3]=gogyo_last[3]-2,gogyo_last[4]=gogyo_last[4]+2
	    end  
	    if im_sm==10
	      gogyo_last[3] -= 1,gogyo_last[4] += 1
	    end  
	...
	if  (sangou_flag==0) && (toritatu_flag==2) && (special_flag==0) #地支に金２個と土１個の場合
	  if gogyo_last[3]>=3
	    ...
	    if im_sm==10
	      gogyo_last[3] -= 1,gogyo_last[4] += 1 #月支が酉（金）で残りが酉（金）辰（土）で化金なので　土-1金+1
	    end  
	  end
	end
	...
	if  (sangou_flag==0) && (toritatu_flag==3) && (special_flag==0) #地支に金１個と土２個の場合
	  if gogyo_last[3]>=3
	    ...
	    if im_sm==5
	      gogyo_last[3] += 1,gogyo_last[4] -= 1 #月支が辰（土）なので　土+1 金-1
	    end  
	  elsif
	    if im_sm==5
	...
	if  (sangou_flag==0) && (uinu_flag==2) && (special_flag==0) #地支に木２個と土１個の場合
	  if gogyo_last[3]>=3
	    ...
	    if im_sm==4
	      gogyo_last[3] -= 1,gogyo_last[1] += 1 #月支が卯（木）で残りが卯（木）戌（土）で化木なので　土-1木+1
	    end  
	  end
	end
	...
	if  (sangou_flag==0) && (uinu_flag==3) && (special_flag==0) #地支に木１個と土２個の場合
	  if gogyo_last[3]>=3
	    if im_sm==4
	      ...
	      if im_sm==11
	        gogyo_last[3] += 1,gogyo_last[1] -= 1 #月支が戌（土）残りが卯（木）戌（土）なので　土+1 木-1
	      end  
	    elsif
	      if im_sm==11
	        gogyo_last[3]=gogyo_last[3]-3,gogyo_last[1]=gogyo_last[1]+3 #月支が戌（土）で残りが戌（土）卯（木）で化木なので　土-3 木+3
	      end  
	      if im_sm==4
	        gogyo_last[3]-2,gogyo_last[1]=gogyo_last[1]+2	  #月支が卯（木）で残りが戌（土）２個で化木なので　土-2木+2
	      end  
	    end
	  end
	end  
	  ...
	  if tenun.exclude?(ym_sn) && tenun.exclude?(ym_se) && tenun.exclude?(ym_sc) && tenun.exclude?(ym_sw) && tenun.exclude?(ym_ss)
	    tenunmei="南天運"
	    tenun_flag=4
	  elsif  tenun.exclude?(ym_sn)&& tenun.exclude?(ym_sc) && tenun.exclude?(ym_ss) &&(tenun.index(ym_se)>=0  || tenun.index(ym_sw)>=0
	    tenunmei="東天運"
	    tenun_flag=3
	  elsif tenun.exclude?(ym_se)  && tenun.exclude?(ym_sc)  && tenun.exclude?(ym_sw)  &&(tenun.index(ym_sn)>=0 || tenun.index(ym_ss)>=0
	    tenunmei="西天運"
	    tenun_flag=2
	  elsif
	    tenunmei="北天運"
	...
	return x
		...
		kst_kan=[0,0,0,0,0]
		k=0
		if   kst_ar.index(1)>=0 && kst_ar.index(4)>=0 && kst_ar.index(5)>=0
			kst_kan[k]=4
			k=k+1
		elsif kst_ar.index(2)>=0 && kst_ar.index(3)>=0 && kst_ar.index(6)>=0
			kst_kan[k]=3
			k=k+1
		elsif  kst_ar.index(3)>=0 && kst_ar.index(6)>=0 && kst_ar.index(7)>=0
			kst_kan[k]=6
			k=k+1
		elsif kst_ar.index(4)>=0 && kst_ar.index(5)>=0 && kst_ar.index(8)>=0
			kst_kan[k]=5
			k=k+1
		elsif kst_ar.index(5)>=0 && kst_ar.index(8)>=0 && kst_ar.index(9)>=0
			kst_kan[k]=8
			k=k+1
		elsif kst_ar.index(6)>=0 && kst_ar.index(7)>=0 && kst_ar.index(10)>=0
			kst_kan[k]=7
			k=k+1
		elsif kst_ar.index(7)>=0 && kst_ar.index(10)>=0 && kst_ar.index(1)>=0
			kst_kan[k]=10
			k=k+1
		elsif kst_ar.index(8)>=0 && kst_ar.index(9)>=0 && kst_ar.index(2)>=0
			kst_kan[k]=9
			k=k+1
		elsif kst_ar.index(9)>=0 && kst_ar.index(2)>=0 && kst_ar.index(3)>=0
			kst_kan[k]=2
			k=k+1
		elsif kst_ar.index(10)>=0 && kst_ar.index(1)>=0 && kst_ar.index(4)>=0
			kst_kan[k]=1
			k=k+1
		end	
		...
		if  kokusen[1]+kokusen[2]+kokusen[3]+kokusen[4]+kokusen[5]+kokusen[6]+kokusen[7]+kokusen[8]+kokusen[9]+kokusen[10] != 0 #刻線が全くない人は才能の星はない。
				if kokusen.index(kokusen.max)!=kokusen.rindex(kokusen.max)
					...
					sainou = d_10shu[kokusen.index(kokusen.max)]+"("+calc_juni(kokusen.rindexkokusen.max),tenun_flag)+"),"+d_10shu[kokusen.index(kokusen.max)]+"("+calc_juni(kokusen.index(kokusen.max),tenun_flag)+")"
				end	
		else
			sainou=""
		end	
		...
		def calc_isoho(a,b,c,d)  #a:im_kd,im_md,im_yd    b:im_sd,im_sm,im_sy     c:taiun_kan   d:taiun_shi  (or c:calc_nenkan  d:calc_nenshi)
			...
			if  (a-c).abs==5 && calc_gousan(b,d)==02)
		      x="干合支合"
			elsif  (a-c).abs==5 && calc_gousan(b,d)==07)
		      x="干合支害"
			elsif  (a-c).abs==5 && calc_gousan(b,d)==05)
		      x="干合支刑"
			elsif  (a-c).abs==5 && calc_gousan(b,d)==08)
		      x="干支合・破"
			elsif  (a-c).abs==5 && calc_gousan(b,d)==09)
		      x="干合合・干刑・冲"
		      ...
		      x="干合合・干刑・破"
			elsif (calc_soukoku(a,c)==03 || calc_soukoku(a,c)==04) && calc_gousan(b,d)==04
		      x="天剋"
			elsif (calc_soukoku(a,c)==03 || calc_soukoku(a,c)==04) && calc_gousan(b,d)==09
		      x="天剋・刑"
			elsif (calc_soukoku(a,c)==01 || calc_soukoku(a,c)==02) && calc_gousan(b,d)==05
		      x="洩天・刑"
			elsif ((calc_soukoku(a,c)==01 || calc_soukoku(a,c)==02) && (b==d)) && calc_gousan(b,d)!=05
		      x="洩天"
			# 18 ←謎
			elsif a==c && b==d
		      x="律音"
		...
		#天剋地冲 検査
		tenkoku_flag=0
		if (calc_soukoku(im_kd,im_ky)==03 || calc_soukoku(im_kd,im_ky)==04) && (im_sy-im_sd).abs==6
		  tenkoku_flag=1  #年・日
		elsif (calc_soukoku(im_kd,im_km)==03 || calc_soukoku(im_kd,im_km)==04) && Math.abs(im_sm-im_sd)==6
		  tenkoku_flag=2 #日・月
		elsif (calc_soukoku(im_km,im_ky)==03 || calc_soukoku(im_km,im_ky)==04) && Math.abs(im_sm-im_sy)==6
		  tenkoku_flag=2  #年・月
		end  
		
		#洩天地支　検査 
		eiten_flag=0
		if (calc_soukoku(im_kd,im_ky)==01 || calc_soukoku(im_kd,im_ky)==02) &&(im_sy==im_sd)
		  eiten_flag=1  #年・日
		elsif (calc_soukoku(im_kd,im_km)==01 || calc_soukoku(im_kd,im_km)==02) &&(im_sm==im_sd)
		  eiten_flag=2 #日・月
		elsif (calc_soukoku(im_km,im_ky)==01 || calc_soukoku(im_km,im_ky)==02) &&(im_sm==im_sy)
		  eiten_flag=2  #年・月
		end  
		...
		shugosin_ar=[["","丁丙庚","丁丙庚","丙癸","庚丙丁戊己","庚丁壬","癸丁庚","癸丁庚","癸庚丁","丁庚壬","丁庚丙","丁甲庚壬癸","庚丁丙戊"]
		...
		tokushu_flag = -1
		tokushu_ar=[]
		if  ar_10shu.index(3)>=0 && ar_10shu.index(9)>=0 && ar_12jyu.index(2)>=0
		        tokushu_flag += 1
		    	tokushu_ar[tokushu_flag]="傷相"
		if  (ar_10shu.index(4)>=0 && ar_10shu.index(9)>=0 && ar_12jyu.index(1)>=0 || (ar_10shu.index(4)>=0 && ar_10shu.index(8)>=0 && ar_12jyu.index(1)>=0
		        tokushu_flag += 1
		    	tokushu_ar[tokushu_flag]="病相"
		if  ar_10shu.index(2)>=0 && ar_10shu.index(9)>=0 && ar_12jyu.index(8)>=0
		        tokushu_flag += 1
		        tokushu_ar[tokushu_flag]="罪相"
		if  (ar_10shu.index(4)>=0 && ar_10shu.index(7))>=0 && (ar_12jyu.index(2)>=0 || ar_12jyu.index(8))>=0
		        tokushu_flag += 1
		     　	tokushu_ar[tokushu_flag]="抗相"
		if  (ar_10shu.index(1)>=0 && ar_10shu.index(1)!=ar_10shu.rindex(1)) && (ar_12jyu.index(1)>=0)
		        tokushu_flag += 1
		     　	tokushu_ar[tokushu_flag]="独相"
		if  (ar_10shu.index(5)>=0 && ar_10shu.index(9)>=0 && ar_12jyu.index(7)>=0) || (ar_10shu.index(5)>=0 && ar_10shu.index(4)>=0 && ar_12jyu.index(7)>=0)
		        tokushu_flag += 1
		    	tokushu_ar[tokushu_flag]="情相"
		if  (ar_10shu.index(5)>=0 && ar_10shu.index(9)>=0 && ar_12jyu.index(7)>=0) || (ar_10shu.index(3)>=0 && ar_10shu.index(6)>=0 && ar_12jyu.index(7)>=0)
		        tokushu_flag += 1
		    	tokushu_ar[tokushu_flag]="色相"
		if  (ar_10shu.index(4)>=0 && ar_10shu.index(9)>=0 && ar_10shu.index(2)>=0 && ar_12jyu.index(8)>=0) 
		        tokushu_flag += 1
		        tokushu_ar[tokushu_flag]="団相"
		if  (ar_10shu.index(5)>=0 && ar_10shu.index(6)>=0 && ar_12jyu.index(12)>=0)
		        tokushu_flag += 1
		        tokushu_ar[tokushu_flag]="破相"
		if  (ar_10shu.index(2)>=0 && ar_10shu.index(6)>=0 && ar_12jyu.index(7)>=0)
		        tokushu_flag += 1
		    	tokushu_ar[tokushu_flag]="消相"
		if  ((ar_10shu.index(4)>=0 && ar_10shu.index(7)>=0 && ar_10shu.index(9)>=0) && ar_12jyu.index(8)>=0)
		        tokushu_flag += 1
		        tokushu_ar[tokushu_flag]="三奇星"
		if  (ar_10shu.index(3)>=0 && ar_10shu.index(9)>=0) || (ar_10shu.index(4)>=0 && ar_10shu.index(7)>=0)
		        tokushu_flag += 1
  ...
  private
    ...
    def calc_zokan(sno, dte)
      @zokan = Zokan.find_by(shi:sno,setsuiri:dte).zokan
      return @zokan
    end
    
    #１０大主星
    def calc_10shu(n, k) 
      @jushu = Jushu.find_by(shi:n,setsuiri:k).jushu
      return @jushu
    end
    
    #１２大従星
    def calc_12jyu(n, k) 
      @junijyu = Junijyu.find_by(shi:n,setsuiri:k).junijyu
      return @junijyu
    end
    
    #合法・散法表
    def  calc_gousan(n, k)
      @gousan = Gousan.find_by(shi:n,setsuiri:k).gousan
      return @gousan
    end
    
    #五行変化表
    def  calc_gogyo_cng(n, k)
      @gogyocng = Gogyocng.find_by(shi:n,setsuiri:k).gogyocng
      return @gogyocng
    end
    
    #半会変化表
    def  calc_gogyo_hankai(n, k)
      @gogyohankai = Gogyohankai.find_by(shi:n,setsuiri:k).gogyohankai
      return @gogyohankai
    end
    
    #支合変化表
    def  calc_gogyo_shigo(n, k)
      @gogyoshigo = Gogyoshigo.find_by(shi:n,setsuiri:k).gogyoshigo
      return @gogyoshigo
    end

    #剋線 算出表
    def  calc_kokusen(n, k)
      @kokusen = Kokusen.find_by(shi:n,setsuiri:k).kokusen
      return @kokusen
    end
    
    #相生・相剋 算出表 干支
    def  calc_soukoku(n, k)
      @soukoku = Soukoku.find_by(shi:n,setsuiri:k).soukoku
      return @soukoku
    end
    
    #四天運と剋線の順位 表
    def  calc_juni(n, k)
      @juni = Juni.find_by(shi:n,setsuiri:k).juni
      return @juni
    end
    
    #蔵干表　裏 数理エネルギー計算用
    def  calc_ura_zokan(n,k)
      @urazokan = Urazokan.find_by(shi:n,setsuiri:k).urazokan
      return @urazokan
    end

    #数理エネルギー     01:木　02:火　03:土　04:金　05:水
    def  calc_suri_gogyo(n,k)
      @surigogyo = Surigogyo.find_by(shi:n,setsuiri:k).surigogyo
      return @surigogyo
    end
    
    #節入り日
    def  calc_setsu(y)
      @setsuiri_str = Setsuiri.find_by(year:y).setsuiri_str
      return @setsuiri_str
    end
